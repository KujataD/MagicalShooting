...
3. `Assets/Scripts/Scene` フォルダに `ScrollingLayerSpawner.cs` という名前で新しいスクリプトを作り、次のコードにします。
   ...
   `scale_`で画像の大きさを変えられます。次回の建物では、この値で奥・中間・手前の3段階を表現します。
   ...
   `spriteRenderer.color`は元の画像に**掛け算**されるため、これで霧を表現すると建物が光って見えます。板を重ねる方式なら、上から色を乗せる形になるので、光らずに白っぽく霞んで見えます。

   ベールの`Sprite`には、白い四角ではなく建物と**同じ画像**（`sprite`）を使っています。四角い板だと、建物の輪郭の外側（隙間から見える空や雲）まで霧色に染まってしまうためです。

4. `Create Empty`から空のオブジェクト`CloudSpawner`を作成し、`ScrollingLayerSpawner`をAdd Componentしてください。座標は (0, 0, 0) のままにします。

   ![Add Componentで検索して追加](add_component_search.png)

5. `ScrollingLayerSpawner`を次のように設定してください。

   - Sprites：`Assets/Images/Cloud`の6枚（`cloud1`〜`cloud6`）
   - Sorting Layer Name：`Cloud`
   - Speed：`1.2`
   - Scale：`0.7`
   - Min Spacing：`3`
   - Align To Ground Y：チェックなし（雲は空のどこにあってもよいため）
   - Distance：`0`（雲はFogの対象外。もともと白いため）

   ![CloudSpawnerの設定](cloud_inspector.png)

   Colorは白のままです。透明度を変えたい場合は、ColorのAlphaを下げてください。

6. Playを押し、雲が画面の外から現れて左へ流れ、前の雲と重ならない間隔を保っていればクリアです。

7. 次回は、このスクリプトで手前に建物を流します。
