...
クリア条件：雲がランダムな種類・位置で左に流れ続ける状態にする

1. 次は雲を流します。スクリプトで次のことを行います。
...
2. `Assets/Scripts/Scene` フォルダに `ScrollingLayerSpawner.cs` という名前で新しいスクリプトを作り、次のコードにします。
...
3. `Create Empty`から空のオブジェクト`CloudSpawner`を作成し、`ScrollingLayerSpawner`をAdd Componentしてください。

   ![Add Componentで検索して追加](add_component_search.png)

4. `ScrollingLayerSpawner`を次のように設定してください。
...
5. Playを押し、雲が画面の外から現れて左へ流れ、前の雲と重ならない間隔を保っていればクリアです。

6. 次回は、このスクリプトで手前に建物を流します。
