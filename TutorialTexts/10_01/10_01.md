...
2. 9章までで、シューティングゲームとして遊べる形が揃いました。10章はチュートリアルではなく、自分の手でゲームを面白くする回です。

3. 追加する内容は自由です。例えば次のようなものがあります。
...
4. 一例として、チャージショットを実装します。スペースキーを押している間チャージし、離した瞬間に、チャージ時間に応じて威力と大きさが増した弾を発射します。

5. `PlayerFire.cs`を次のように書き換えてください。
...
6. Playを押し、スペースキーを軽くタップしたときと長押ししたときで、弾の大きさと威力が変わればクリアです。最大チャージの一撃で、通常は3回当てないと倒せない敵を倒せます。

   ![チャージショットで敵を一撃で撃破](charge_shot_confirm.png)

7. これで10章は完了です。この例を参考に、自分のアイデアを足していきましょう。
