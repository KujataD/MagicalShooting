...
5. Canvasの子として「UI > Image」でハートを1個作り、名前を`Heart1`にします。Source Imageは特に設定せず、白い四角のままにしておきます。

   ![Heart1を作成した状態](heart1_created.png)

   このチュートリアルではハートの絵は用意していないので、白い四角のまま進めます。余裕があれば、あとで自分でハートの絵を描いて`Source Image`に設定してみましょう。
