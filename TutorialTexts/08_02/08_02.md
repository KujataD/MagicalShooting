...
5. Canvasの子として「UI > Image」でハートを1個作り、名前を`Heart1`にします。`Width: 48, Height: 48`にし、Source Imageは特に設定せず、`Color`を赤（`R: 255, G: 0, B: 0`）にします。ハート1個の大きさは、あとで並べる`HeartContainer`の幅（`160`）の計算にも使います。
