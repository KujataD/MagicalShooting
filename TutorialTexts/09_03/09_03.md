...
3. `PlayerHealth.cs`に、被弾音のフィールドと処理を追加します。
...
4. `Player`の`Player Health`の`Audio Source_`に9-02で追加した`Audio Source`を、`Damage Se_`に`hit`を設定してください。

   ![Player HealthのAudio Source_を設定](health_audiosource_wired.png)

   ![Damage Se_にhitを設定](damage_se_wired.png)

5. Playを押し、敵に当たってHPが減るたびに被弾音が鳴ればクリアです。
