...
6. もう一度複製して `BackgroundNear` を作り、次のように変えます。
   ...
   Order In Layerを0・2・4と分けているのは、同じ`Background`レイヤーの中で手前のものが奥のものを隠すように描画順を揃えるためです。数字が大きいほど手前に描かれます。1つおきにしているのは、6-04の霧の板が建物の1つ上（Order In Layer＋1）に描かれるためで、隣のレイヤーの建物と番号が重ならないようにしています。
...
8. Playを押して確認します。奥の小さい建物、中間の建物、手前の大きい建物が、それぞれ違う速さで流れているのが見えるはずです。3つとも地面にきちんと接地していることも確認します。
